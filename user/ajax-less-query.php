...
$fetchQuery = mysqli_fetch_array($insertQuery);
  if(mysqli_num_rows($insertQuery)>0){
  	if(strlen($fetchQuery['customer_enquiry'])>100){
  	$error_msg['no_error'] = substr($fetchQuery['customer_enquiry'],0,100)."...  <a href='javascript:;'  onclick='view_more(".$fetchQuery[0].",".$_POST['countid'].")'><i class='fa fa-plus' style='color:blue;'></a>";
  	}
  }else{
