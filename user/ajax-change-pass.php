...
extract($_POST);
$selectPass = mysqli_query($conn,"SELECT * FROM `company_reg` WHERE id = '".$_POST['profile_id']."' AND user_org_pass = '".$_POST['c_pass']."' ");
if(mysqli_num_rows($selectPass) == 0){
	$error_msg['main_error'] = "Enter your valid current password";
}elseif($_POST['n_pass'] == ''){
	$error_msg['main_error'] = "Enter your new password";
}
else if($_POST['n_pass'] != $_POST['cn_pass']){
	$error_msg['main_error'] = "Confirm password doesn't matched with new password";
}else{
  $insertQuery = mysqli_query($conn,"UPDATE `company_reg` SET user_org_pass = '".$_POST['n_pass']."', user_pass = '".md5($_POST['n_pass'])."' WHERE id = '".$_POST['profile_id']."' ");
  if($insertQuery){
