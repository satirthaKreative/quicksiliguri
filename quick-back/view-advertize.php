...
<?php if(isset($_GET['del_id'])){
  $deleteAdvertize = mysqli_query($conn,"DELETE FROM `advertize` WHERE id = '".$_GET['del_id']."' ");
  if($deleteAdvertize){
    echo "<script>window.location.href='view-advertize?sucmsg=Data deleted successfuly '</script>";
  } else{
    echo "<script>window.location.href='view-advertize?errmsg=Something went error!Try again later'</script>";
  }
} ?>
            ...
            <div class="breadcrumbs-top float-md-right">
              <div class="breadcrumb-wrapper mr-1">
                <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item active">Promotional Advertize
                  </li>
                </ol>
              </div>
            </div>
          ...
          <div class="row">
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <h4 class="card-title">Promotional Advertize</h4>
                ...
                <div class="card-content collapse show">
                  <div class="col-lg-6 col-lg-offset-3">
                    <?php if(isset($_GET['sucmsg'])){ ?>
                        <p style="color: green;" class="showDelmsg"><i class="fa fa-check"></i><?= $_GET['sucmsg']; ?></p>
                    <?php } else if(isset($_GET['errmsg'])){ ?>
                        <p style="color: red;" class="showDelmsg"><i class="fa fa-times"></i><?= $_GET['errmsg']; ?></p>
                    <?php } ?>
                  </div>
                  <div class="card-body">
                    <!-- <div class="row">
                  ...
                  <div class="table-responsive">
                    <table class="table">
                      ...
                      <tbody class="main_show">
                        <?php $i = 1;
                        $review_admin1 = mysqli_query($conn,"SELECT * FROM `advertize` ORDER BY id DESC ");
                        ...
                        $select_business_list = mysqli_query($conn,"SELECT * FROM `advertize` ORDER BY id DESC LIMIT ".($pages-1)*$page_view.", ".$page_view." ");
                        if(mysqli_num_rows($select_business_list)>0){
                         while($fetch_company_list = mysqli_fetch_array($select_business_list)){ ?>
                        <tr>
                          <th scope="row"><?= $i; ?></th>
                          <th scope="row"><?= $fetch_company_list['business_name']; ?></th>
                          <td><?php if($fetch_company_list['p_image'] !='' ){ ?><img src="uploads/promotional/images/<?= $fetch_company_list['p_image']."?autoplay=1"; ?>" height="150"> <?php } ?></td>
                          
                          <td><?php if($fetch_company_list['p_video'] !='' ){ ?>
                              <iframe width="200" height="100" src="<?= $fetch_company_list['p_video']; ?>" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                             <?php } ?></td>
                          <td>
                            <?= $fetch_company_list['pay_type']; ?>
                          </td>
                          <?php if(isset($_SESSION['login_id'])){ ?>
                          <td><?php if($fetch_company_list['status'] == 1){ ?><button type="button" class="btn btn-sm btn-info" onclick="change_activity(0,<?= $fetch_company_list['id'] ?>)">Approved</button> <?php }else if($fetch_company_list['status'] == 0){ ?> <button type="button" onclick="change_activity(1,<?= $fetch_company_list['id'] ?>)" class="btn btn-sm btn-warning">Not Approved</button>
                            <?php } ?></td>
                            <?php } ?>
                            <td><a href="view-advertize?del_id=<?= $fetch_company_list['id']; ?>" class="text-danger" onclick='return confirm("Are you want to delete this ?")'><i class="fa fa-trash"></i></a></td>
                        </tr>
                        <?php $i++; } } else{ ?>
                          <?php if(isset($_SESSION['emp_id'])){ ?>
                            <td colspan="6">No promosional vedio added before</td>
                          <?php } if(isset($_SESSION['login_id'])){ ?>
                            <td colspan="7">No promosional vedio added before</td>
                          <?php } ?>
                        <?php } ?>
                      </tbody>
                      <tbody id="ajax_show">
            ...
            <div class="col-12 main_show" >
              <div style="text-align: center;">
                <?php for ($i=1; $i <= $total_page; $i++) {  ?><a href="view-advertize?pages=<?= $i; ?>" <?php if($pages==$i){ ?> class="active" <?php } ?> id="page_id"><?= $i; ?></a> &nbsp;
                <?php } ?>
              </div>
            </div>
          </div>
                ...
                <div class="modal-body">
                  <form id="premium_plans">
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control" name="payment_types" id="payment_types" required="required">
      ...
      success: function(event){
        if(event.no_error){
          ...
          window.setTimeout(function() {
              window.location.href = 'view-advertize.php';
          }, 1000);
